...
print('--------ЗАДАЧА №1--------')
if (user_role == "admin" or has_api_key) and model_available \
...
if min(reviews_sentiments) < 0:
    print('Есть хотя бы один негативный')
...
if user_tier == "premium" and subscription_active \
        and has_any_premium_model \
        and user_quota["used"] < user_quota["total"]:
    print('Есть доступ к премиум-функциям')
